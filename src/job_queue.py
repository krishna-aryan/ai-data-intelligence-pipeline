...
class JobExecutionError(RuntimeError):
    def __init__(self, message: str, *, category: str = "job_failure", retryable: bool = False) -> None:
                ...
                try:
                    ...
                    try:
                        ...
                        if retryable and attempt < max_retries:
                            metrics["retried"] += 1
                            await queue.put((job, attempt + 1))
                            continue
                        result_slots[job.job_id] = JobResult(job=job, status="failed", attempt_count=attempt + 1, error_category=category, error_message=str(exc), started_at=start_time)
                        metrics["failed"] += 1
                    else:
